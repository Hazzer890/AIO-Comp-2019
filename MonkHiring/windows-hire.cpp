...
int main(void) {
    ...
    for(int i = 0; i < N; i++){
        for(int a = 0; a < S; a++){
            if ((monk_skills[i] <= student_job_skills[a]) && (monk_skills[i] != -1) && (student_job_skills[a] != -5)){
                answer++;
                monk_skills[i] = -1;
                student_job_skills[a] = -5;
            }
        }
        for(int a = 0; a < M; a++){
            if ((monk_skills[i] >= master_job_skills[a]) && (monk_skills[i] != -1) && (master_job_skills[a] != -5)){
                answer++;
                monk_skills[i] = -1;
                master_job_skills[a] = -5;
            }
        }
    }
